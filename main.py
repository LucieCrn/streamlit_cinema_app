...
# Importer les données pour l'onglet "watchlist" et suppression des
# colonnes inutiles
...
def onglet_films():
    ...
    st.write(
        '<h1 style="text-align:center;color:#FFDD99;font-weight:bolder;font-size:60px;">MES FILMS 🍿</h1>',
...
# OUTILS DE SELECTION - INTERRACTION AVEC L'UTILISATEUR
    ...
    with col1:
        ...
        selected_year = st.selectbox(
            'Choisir une année de visionnage', years_list)
    ...
    with col3:
        choix_affiche = st.selectbox(
            "Afficher les affiches des films (attention très lent)", [
                "Non", "Oui"])

    # Création de la selection des notes.
    selected_rates = st.slider(
        'Afficher les notes comprises entre :', 0, 10, (0, 10))

    # Afficher le nombre de film
    st.markdown("---")
    # Appliquer les filtres à la dataframe
    nombre_films = fct.filtre_table(
        mes_films,
        selected_year,
        selected_genre,
        selected_rates).shape[0]
...
# CREATION DES GRAPHIQUES ET DE LA VISUALISATION DE LA TABLE
    ...
    with col1:
        mes_films_filtre = fct.filtre_table(
            mes_films, selected_year, selected_genre, selected_rates)
        fig, ax = plt.subplots()
        ax.hist(
            mes_films_filtre["Your Rating"],
            color="pink",
            edgecolor="black")
    ...
    if choix_affiche == "Oui":
        ...
        for film in mes_films_liste:
            ...
            # Organiser les infos en colonnes avec affiche + nom du film
            # dessous avec lien cliquable
            x = x + 1
            if x > 4:
                x = x - 4
...
def onglet_watchlist():
    ...
    # Titre de la page
    st.write(
        '<h1 style="text-align:center;color:purple;font-weight:bolder;font-size:60px;">MA WATCHLIST 🎥</h1>',
        unsafe_allow_html=True)
    ...
    with col2:
        ...
        selected_year = st.selectbox(
            'Choisir une année de réalisation',
            years_watchlist)

    # Création de la selection des notes.
    selected_rates = st.slider(
        'Afficher les notes IMBd comprises entre :', 0, 10, (0, 10))

    # Création de la DF filtrée.
    watchlist_filtre = fct.filtre_watchlist(
        watchlist, selected_genre, selected_year, selected_rates)
    ...
    for film in watchlist_films:
        ...
        # Organiser les infos en colonnes avec affiche + nom du film dessous
        # avec lien cliquable
        x = x + 1
        if x > 3:
            x = x - 3
...
# COLONNE POUR LA SELECTION DE LA PAGE
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown(
        '<a href="http://allocine.fr">Lien vers le site Allociné</a>',
        unsafe_allow_html=True)
    st.sidebar.markdown(
        '<a href="http://imdb.com">Lien vers le site IMDb</a>',
        unsafe_allow_html=True)
