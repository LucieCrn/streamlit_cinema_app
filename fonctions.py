...
def get_genres(data):
    liste_des_genres = set(str(data["Genres"].tolist()).replace(
...
def filtre_watchlist(data, selected_genre, selected_year, selected_rates):
    ...
    mes_films_notes = mes_films_year.loc[(mes_films_year["IMDb Rating"] >= min(
        selected_rates)) & (mes_films_year["IMDb Rating"] <= max(selected_rates))]
    ...
    if selected_genre == "All":
        ...
        for ligne in range(0, len(mes_films_notes)):
            if str(selected_genre) in mes_films_notes.iloc[ligne]["Genres"]:
                mes_films_genre = mes_films_genre.append(
                    mes_films_notes.iloc[ligne])
            else:
    ...
    watchlist_filtres["IMDb Rating"] = watchlist_filtres["IMDb Rating"].astype(
        int)
    watchlist_filtres["Year"] = watchlist_filtres["Year"].astype(int)
    watchlist_filtres["Runtime (mins)"] = watchlist_filtres["Runtime (mins)"].astype(
        int)
...
def filtre_table(data, selected_year, selected_genre, selected_rates):
    ...
    # Application du filtre "notes"
    mes_films_notes = mes_films_year.loc[(mes_films_year["Your Rating"] >= min(
        selected_rates)) & (mes_films_year["Your Rating"] <= max(selected_rates))]
    ...
    mes_films_filtres = mes_films_genre.drop(
        ["Unnamed: 0", "URL", "Date Rated", "IMDb Rating"], axis=1)
    mes_films_filtres["Your Rating"] = mes_films_filtres["Your Rating"].astype(
        int)
    mes_films_filtres["Year"] = mes_films_filtres["Year"].astype(int)
    mes_films_filtres["Runtime (mins)"] = mes_films_filtres["Runtime (mins)"].astype(
        int)
